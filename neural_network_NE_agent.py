...
class NNM_1_Cascading_random:
    def __init__(self, input_layer, neurons_in_layer_n, output_layer):
        ...
        while self.neurons_in_next_layer > output_layer:  # check var is assigned a higher var than output_layer to start the loop
            self.neurons_in_layer = self.neuron_number[self.loop_counter] / 2
            self.neurons_in_next_layer = self.neurons_in_layer / 2
...
class Evolution:
    def __init__(self):
        ...
        self.generation = first_generation(genomes, 8, 16, 4)
    ...
    def create_generation(self):
        ...
        number_of_copied_genomes = math.floor(
            (number_of_genomes - len(self.child_neural_net_weights)) * percentage_copied)
        number_of_random_genomes = math.floor(
            (number_of_genomes - len(self.child_neural_net_weights)) * percentage_random)

        while (number_of_random_genomes + number_of_copied_genomes + len(
                self.child_neural_net_weights)) > number_of_genomes:
